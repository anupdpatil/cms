...
$name_format = (isset($_GET['name_format'])) ? $_GET['name_format'] : 1;
...
$sql = "SELECT "
        ...
        . "JOIN repayment r on r.admission_id=a.id $condition";
$result = [];
...
$sql = "SELECT active_admission_id from students where active_class='$selected_course'";
$stds = [];
if ($res = mysqli_query($link, $sql)) {
    while ($user_data = mysqli_fetch_assoc($res)) {
                                ...
                                <div class="row">
                                    <div class="col-xs-12">
                                        <div class="table-header">
                                            <!--                                            Results for Admissions of 
                                                                                        <div class="form-group">-->
                                            <label class="col m-l-1" for="course">Course: </label>
                                            <select name="course" id="course" onchange="getData()">
                                                <?php for ($index = 0; $index < count($course); $index++) { ?>
                                                    <?php if ($course[$index]['id'] == $selected_course) { ?>
                                                        ...
                                                        <tr>
                                                            <td>
                                                                <?php if (($_SESSION['user'] == 'Anup Patil') || ($_SESSION['user'] == 'Jayram Chaudhari')) { ?>
                                                                    <a onclick="confirmDelete(<?php echo $result[$index]['id']; ?>)" href="#">
                                                            ...
                                                            <?php if (($_SESSION['user'] == 'Anup Patil')) { ?>
                                                                ...
                                                                <td>
                                                                    <?php echo $result[$index]['fee_type']; ?>
                                                                </td>
                                                            <?php } ?>
                                                            <td>
                                                                <?php
                                                                ...
                                                                for ($a = 0; $a < count($r); $a++) {
                                                                    if (($r[$a]->header == 'TUITION FEE') || ($r[$a]->header == 'TUTION FEE') || ($r[$a]->header == 'ADD FEE') || ($r[$a]->header == 'TERM FEE') || ($r[$a]->header == 'ADMISSION FEE') || ($r[$a]->header == 'ADMISSION FFE') || ($r[$a]->header == 'LIBRARY FEE') || ($r[$a]->header == 'MEDICAL FEE') || ($r[$a]->header == 'GYMKHANA FEE') || ($r[$a]->header == 'POOR STUD AID FUND') || ($r[$a]->header == 'STUDENTS ACTIVITY FEE') || ($r[$a]->header == 'LABORATORY FEE') || ($r[$a]->header == 'STUDENTS GROUP INS (UNI)') || ($r[$a]->header == 'COMPUTERIZATION FEE') || ($r[$a]->header == 'COLLEGE DEVELOPMENT FUND') || ($r[$a]->header == 'ASHWAMEDH FEE') || ($r[$a]->header == 'IDENTITY FEE') || ($r[$a]->header == 'YUVA RANG/YOUTH FESTIVEL') || ($r[$a]->header == 'YUVA RANG/YOUTH FESTIVAL')) {
                                                            ...
                                                            <td class="hidden-480">
                                                                <?php if ($_SESSION['user'] == "Anup Patil") { ?>
                                                                    <a class="btn btn-danger btn-xs m-1 cancel_admission" id="<?php echo $result[$index]['id']; ?>">Cancel Admission</a>
                                                                <?php } ?>

                                                                <?php if (($applicable_fees - $paid_amount) > 0) { ?>
                                                                    <a class="btn btn-warning btn-xs m-1 pay_dues" id="<?php echo $result[$index]['id']; ?>&class=<?php echo $result[$index]['class']; ?>">Pay Dues</a>
                                                                    <?php
                                                                }
                                                                if ($result[$index]['next_course_admission_open']) {
                                                                    ?>
                                                                    <a href="admission-to-next-year.php?form=<?php echo $result[$index]['id']; ?>&class=<?php echo $result[$index]['next_course']; ?>&group=<?php echo $result[$index]['course_group']; ?>" class="<?php if ($result[$index]['next_course'] == 0) { ?> disabled btn-default <?php } else { ?> btn-success <?php } ?> btn btn-xs">Promote</a>
                                                                    <?php
                                                                }
                                                                if ((!in_array($result[$index]['id'], $students)) && ($_SESSION['user'] == "Anup Patil")) {
                                                                    ?>
                                                                    <a href="#" class="btn btn-default btn-xs m-1 get-student-info" id="<?php echo $result[$index]['id']; ?>">Verify Student</a>
                                                                    <a href="#" class="btn btn-danger btn-xs m-1 cancel-admission" id="<?php echo $result[$index]['id']; ?>">Cancel Admission</a>
                                                                <?php } ?>
                                                            </td>
                                                            <td>
                                                                <?php
