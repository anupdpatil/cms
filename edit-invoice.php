...
$result = [];
if (!empty($_POST)) {
    $invoice_data = $_POST['invoice_details'];
    $invoice_number = $_POST['invoice_number'];

    $date = date('Y-m-d H:i:s');
    $sql = "UPDATE invoices SET data = '$invoice_data' WHERE id ='$invoice_number'";
    if (mysqli_query($link, $sql)) {
        $url = $_SERVER['SERVER_NAME'];
        if ($_SERVER['SERVER_NAME'] == 'localhost')
            header("Location: http://localhost/acsc/admissions.php?course=".$_GET['class']);
        else
            header("Location: http://" . $url . "/admissions.php?course=".$_GET['class']);
    } else {
...
if (isset($_GET['invoice_id'])) {
    $sql = "SELECT id, data from invoices where id=" . $_GET['invoice_id'];
                                ...
                                <div class="col-xs-12">
                                    <form action="" method="post" class="form-horizontal" role="form" id="invoice_form">
                                        <div class="form-group">
                                            <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Invoice Headers </label>
                                            <div class="col-sm-9">
                                                <input value="<?php echo $result[0]['id']; ?>" name="invoice_number" type="text" id="invoice_number" placeholder="invoice number"/>
                                                <textarea name="invoice_details" rows="15" cols="80" id="myTextArea"><?php echo $result[0]['data'];?></textarea>
                                            </div>
                                        </div>
