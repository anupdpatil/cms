...
if (!empty($_POST)) {
        $link = config();
    $course_info = split("\&", $_POST['course']);
    $class = $course_info[0];
    ...
    if (mysqli_query($link, $sql)) {
        if ($_SERVER['SERVER_NAME'] == 'localhost')
            header('Location: http://localhost/acsc/create-invoice.php?class=' . $class . '&form=' . $link->insert_id . '&' . $course_group);
        else
            header('Location: https://' . $_SERVER['SERVER_NAME'] . '/create-invoice.php?class=' . $class . '&form=' . $link->insert_id . '&' . $course_group);
    } else {
        echo "ERROR: Could not able to execute $sql. " . mysqli_error($link);
        die();
    }
                                                ...
                                                <span class="block input-icon input-icon-right">
                                                    <select name="course" id="course" class="col-xs-10 col-sm-7">
                                                        ...
                                                        <?php for ($index = 0; $index < count($result); $index++) {
                                                            ?>
                                                            <option value="<?php echo $result[$index]['id']; ?>&group=<?php echo $result[$index]['course_group']; ?>"><?php echo $result[$index]['course_name']; ?></option>
                                                        <?php }
                                                        ?>
                                                    </select>
                                                </span>
