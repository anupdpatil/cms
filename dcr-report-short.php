...
$user_data = [];
    ...
    $link = config();
$sql = "SELECT id from user where role_id=2";
$counters = [];
if ($res = mysqli_query($link, $sql)) {
    while ($user_data = mysqli_fetch_assoc($res)) {
        array_push($counters, $user_data);
    }
} else {
...
if (isset($_GET['course_group']) && isset($_GET['date_from']) && isset($_GET['date_to'])) {
    ...
    $counter_condition = (isset($_GET['counter'])) ? " AND ad.created_by IN (SELECT CONCAT_WS(' ',user.first_name, user.last_name) from user WHERE user.id ='" . $_GET['counter'] . "') " : '';
    $sql = "SELECT ad.*,
        cr.course_name, 
        cr.fees_breakup, 
        ...
        $counter_condition
        AND ad.created_date between '" . $st_date . "' and '" . $end_date . "' ORDER BY inv.invoice_no";
                ...
                <div class="main-content-inner">
                    <div class="breadcrumbs breadcrumbs-fixed  invisible-row" id="breadcrumbs">
                        <ul class="breadcrumb">
                            ...
                            <li class="active">Short DCR Report</li>
                        </ul>
                    </div>
                                ...
                                <div class="row invisible-row">
                                    <div class="invisible-row" style="padding: 10px;">
                                        <form class="form-inline">
                                            ...
                                            <div class="form-group mx-sm-3 mb-2 col-sm-offset-1">
                                                <label>Counter </label>
                                                <select name="counter" id="counter" onchange="getData()">
                                                    <option value="">---Counter---</option>
                                                    <?php for ($i = 0; $i < count($counters); $i++) { ?>
                                                        <?php if (isset($_GET['counter']) && ($_GET['counter'] == $counters[$i]['id'])) { ?>
                                                            <option  selected="true" value="<?php echo $counters[$i]['id']; ?>"><?php echo $counters[$i]['id']; ?></option>
                                                        <?php } else { ?>
                                                            <option value="<?php echo $counters[$i]['id']; ?>"><?php echo $counters[$i]['id']; ?></option>
                                                            <?php
                                                        }
                                                    }
                                                    ?>
                                ...
                                <?php if (!empty($headers)) { ?>
                                    ...
                                    <!--<div class="row">-->
                                        <div class="table-header">
                                            <b>Short DCR Report : <?php echo $_GET['course_group'] ?> College (<?php echo date('d-M-Y', strtotime($_GET['date_from'])); ?>  &nbsp;to&nbsp; <?php echo date('d-M-Y', strtotime($_GET['date_to'])); ?>) | Counter: <?php echo (isset($_GET['counter']))? $_GET['counter'] :"All";  ?></b>
                                                    ...
                                                    if (isset($data)) {
                                                        for ($index = 0; $index < count($data); $index++) {
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td><?php echo strtoupper($data[$index]['course_name']); ?></td>
...
<![endif]-->
        ...
        <script type="text/javascript">
                                                function getData() {
                                                    ...
                                                    } else if ($('#group').val() && $('#date_range_from').val() && ($('#date_range_to').val())) {
                                                        window.location.replace("?course_group=" + $('#group').val() + "&date_from=" + $("#date_range_from").val() + "&date_to=" + $("#date_range_to").val() + "&counter=" + $('#counter').val());
                                                    }
                                                }
                                                function selectDate() {
